...
// Scan the block chain (starting in pindeXGCart) for transactions
// from or to us. If fUpdate is true, found transactions that already
// exist in the wallet will be updated.
int CWallet::ScanForWalletTransactions(CBlockIndex* pindeXGCart, bool fUpdate)
{
    int ret = 0;

    CBlockIndex* pindex = pindeXGCart;
    {
        LOCK(cs_wallet);
        while (pindex)
        {
...
bool CWallet::UnlockStealthAddresses(const CKeyingMaterial& vMasterKeyIn)
{
    ...
    for (; mi != mapCryptedKeys.end(); ++mi)
    {
        ...
        if (cpkT != pubKey)
        {
            ...
            if (fDebug)
            {
                printf("cpkT   %s\n", HeXGCr(cpkT.Raw()).c_str());
                printf("pubKey %s\n", HeXGCr(pubKey.Raw()).c_str());
            };
            continue;
        };
    ...
    BOOST_FOREACH(const CTxOut& txout, tx.vout)
    {
        ...
        BOOST_FOREACH(const CTxOut& txoutB, tx.vout)
        {
            ...
            for (it = stealthAddresses.begin(); it != stealthAddresses.end(); ++it)
            {
                ...
                //printf("pkExtracted %"PRIszu": %s\n", pkExtracted.size(), HeXGCr(pkExtracted).c_str());
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64 nSearchInterval, CTransaction& txNew)
{
    ...
    BOOST_FOREACH(PAIRTYPE(const CWalletTx*, unsigned int) pcoin, setCoins)
    {
        ...
        static int nMaXGCakeSearchInterval = 60;

        // printf(">> block.GetBlockTime() = %"PRI64d", nStakeMinAge = %d, txNew.nTime = %d\n", block.GetBlockTime(), nStakeMinAge,txNew.nTime);
        if (block.GetBlockTime() + nStakeMinAge > txNew.nTime - nMaXGCakeSearchInterval)
            continue; // only count coins meeting min age requirement

        bool fKernelFound = false;
        for (unsigned int n=0; n<min(nSearchInterval,(int64)nMaXGCakeSearchInterval) && !fKernelFound && !fShutdown; n++)
        {
            // printf(">> In.....\n");
            // Search backward in time from the given txNew timestamp
            // Search nSearchInterval seconds back up to nMaXGCakeSearchInterval
