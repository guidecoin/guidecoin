...
void ScriptPubKeyToJSON(const CScript& scriptPubKey, Object& out)
{
    ...
    out.push_back(Pair("hex", HeXGCr(scriptPubKey.begin(), scriptPubKey.end())));
...
void TxToJSON(const CTransaction& tx, const uint256 hashBlock, Object& entry)
{
    ...
    BOOST_FOREACH(const CTxIn& txin, tx.vin)
    {
        Object in;
        if (tx.IsCoinBase())
            in.push_back(Pair("coinbase", HeXGCr(txin.scriptSig.begin(), txin.scriptSig.end())));
        else
        {
            ...
            o.push_back(Pair("hex", HeXGCr(txin.scriptSig.begin(), txin.scriptSig.end())));
...
Value getrawtransaction(const Array& params, bool fHelp)
{
    ...
    ssTx << tx;
    string strHex = HeXGCr(ssTx.begin(), ssTx.end());
...
Value listunspent(const Array& params, bool fHelp)
{
    ...
    if (params.size() > 2)
    {
        ...
        BOOST_FOREACH(Value& input, inputs)
        {
            ...
            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid guidecoin address: ")+input.get_str());
    ...
    BOOST_FOREACH(const COutput& out, vecOutputs)
    {
        ...
        entry.push_back(Pair("scriptPubKey", HeXGCr(pk.begin(), pk.end())));
...
Value signrawtransaction(const Array& params, bool fHelp)
{
    ...
    CDataStream ssTx(SER_NETWORK, PROTOCOL_VERSION);
    ssTx << mergedTx;
    result.push_back(Pair("hex", HeXGCr(ssTx.begin(), ssTx.end())));
