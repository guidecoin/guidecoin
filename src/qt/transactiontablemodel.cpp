...
QString TransactionTableModel::formatTXGCatus(const TransactionRecord *wtx) const
{
    QString status;
...
QVariant TransactionTableModel::tXGCatusDecoration(const TransactionRecord *wtx) const
{
    if(wtx->type == TransactionRecord::Generated || wtx->type == TransactionRecord::StakeMint)
    {
        switch(wtx->status.maturity)
...
{
    QString tooltip = formatTXGCatus(rec) + QString("\n") + formatTxType(rec);
