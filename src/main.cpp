...
unsigned int nStakeMinAge = 60 * 60 * 24 * 1;	//minimum age for coin age:  3 day
...
int64 nChainStartTime = 1424693142;
...
const string strMessageMagic = "guidecoin Signed Message:\n";
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in guidecoin
    bool fStrictPayToScriptHash = true; // Always active in guidecoin
...
if (!CheckCoinStakeTimestamp(GetBlockTime(), (int64)vtx[1].nTime))
    ...
    if ((nTime >= CUTOFF_POW_TIME) && (IsProofOfWork()))
         return DoS(100, error("CheckBlock() : Proof of work (%f XGC) at t=%d on or after %d.\n",
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "guidecoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        block.nVersion = 1;
        block.nTime    = 1424693142;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 0; // perfect nonce
